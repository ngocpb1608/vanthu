...
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False  # tránh warning

PAGE_SIZE = int(os.getenv('PAGE_SIZE', '50'))  # phân trang cho danh sách
...
def admin_required(view):
    """Chặn các route chỉ dành cho ADMIN."""
    @wraps(view)
...
def bootstrap():
    ...
    # Seed demo nếu SEED_DEMO=1 và bảng công văn đang trống
    if os.getenv('SEED_DEMO', '0') == '1' and CongVan.query.count() == 0:
        samples = [
            CongVan(ma='CV-001', loai_don_thu='Khiếu nại', thang=1, nam=2025, ma_kh='KH01',
                    ten='Nguyễn Văn A', dia_chi='Hà Nội', nhan_vien='Lan',
                    noi_dung='Khiếu nại về thủ tục cấp sổ.', ngay_nv_nhan=date(2025, 1, 5),
                    tinh_trang='Đang giải quyết'),
            CongVan(ma='CV-002', loai_don_thu='Đề nghị', thang=1, nam=2025, ma_kh='KH02',
            ...
            CongVan(ma='CV-003', loai_don_thu='Phản ánh', thang=12, nam=2024, ma_kh='KH03',
...
def inject_globals():
    """Biến dùng chung + helper tạo URL phân trang giữ nguyên bộ lọc."""
    def page_url(p):
...
def _query_congvan_from_request():
    ...
    loai = request.args.get('loai', '').strip()   # dropdown Loại
    kw = request.args.get('q', '').strip()        # từ khoá nội dung

    if ma:
        q = q.filter(CongVan.ma.ilike(f'%{ma}%'))
    if ten:
        q = q.filter(CongVan.ten.ilike(f'%{ten}%'))
    if dia_chi:
        q = q.filter(CongVan.dia_chi.ilike(f'%{dia_chi}%'))
    if thang.isdigit():
        q = q.filter_by(thang=int(thang))
    if nam.isdigit():
        q = q.filter_by(nam=int(nam))
    if tinh_trang:
        q = q.filter_by(tinh_trang=tinh_trang)
...
def quick_login():
    """Đăng nhập nhanh: tạo/đăng nhập user staff chỉ-xem."""
    u = User.query.filter_by(username='quick_staff').first()
    if not u:
        u = User(username='quick_staff', role='staff')
        u.set_password('quick')
        db.session.add(u)
        db.session.commit()
    login_user(u)
    flash('Đăng nhập nhanh (quyền xem).', 'success')
    return redirect(url_for('dashboard'))
...
def dashboard():
    ...
    hoan_thanh_count = db.session.query(func.count(CongVan.id)).filter_by(tinh_trang='Hoàn Thành').scalar()

    # danh sách theo bộ lọc + phân trang
    page = request.args.get('page', 1, type=int)
    query = _query_congvan_from_request().order_by(CongVan.created_at.desc())

    # Flask-SQLAlchemy 3.x: dùng db.paginate (accept Query hoặc Select)
...
def congvan_new():
    if request.method == 'POST':
        try:
            ...
            if CongVan.query.filter_by(ma=ma).first():
                raise ValueError('Mã công văn đã tồn tại.')
            ...
            cv = CongVan(
                ...
                thang=_safe_int(request.form.get('thang')),
                nam=_safe_int(request.form.get('nam')),
...
def congvan_edit(id):
    ...
    if request.method == 'POST':
        try:
            new_ma = request.form['ma'].strip()
            if new_ma != item.ma and CongVan.query.filter_by(ma=new_ma).first():
                raise ValueError('Mã công văn đã tồn tại.')
...
def export_excel():
    ...
    buf.seek(0)
    return send_file(
        buf, as_attachment=True, download_name='congvan.xlsx',
